...
masodF2 a b c = if delta < 0 then error "komplex szamok" else (gy1, gy2)
  where
    delta = b ** 2 - 4 * a * c
    gy1 = (-b + sqrt delta) / (2 * a)
    gy2 = (-b - sqrt delta) / (2 * a)
...
hatvanyX :: (Ord a, Floating a) => a -> a -> a
hatvanyX x n
...
main = do
  ...
  let ep1 = (6, 7)
      ep2 = (4, 7)
  putStrLn ("elempar " ++ show ep1 ++ " " ++ show ep2 ++ show (elempar2 ep1 ep2))
  print (tf2 10)